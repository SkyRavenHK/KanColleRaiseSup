...
if(typeof(dpnla) == 'undefined'){
	var dpnla = {
		...
		init: function(){
		/* 初期化 */
			let $tabs = $('.container-fluid nav');
		//	console.log($tabs);
		//	$tabs.forEach(this.Tab_Init);
			...
			console.log('finished init tabs');
			this.data['pmbcloserunproc'] = null;		this.matupdinit();
			console.log('finished updating materials');
			$("#pmb02").click(() => {dpnla.pmbclose();});
			$("#b563c").click(() => {dpnla.kcprecls();});
			console.log('finished setting delete records events');
		...
		* 1:ページの(←)マイナス(→)プラス…0)なし、1)あり // with navigation arrows
		* 2:２ヶ所の画面切替が…0)なし、1)あり // switches 2 'panels' at the same time
		*/
			...
			this.data[id +'_fg'] = fg;
			var ky = id +'_ns';		var ns = this.data[ky];
			if(ns == undefined) this.data[ky] = 0;
			var tb = $('#'+id).find('a');
			console.log(tb);
		//	var tb = this.getn(this.ge(id),'li');
			this.data[id +'_li'] = tb;		var mx = tb.length;
			for(i = 0;i < mx;i++){
				j = i + 1;	ky = id +'_'+ j;
				if(i > 0) this.tabdyset(ky,'none',fg);
				$(tb[i]).click((e) => {dpnla.tabClick(e); return dpnla.eventCancel(e);});
			//	this.addevent(tb[i],'click',function(e){ dpnla.tabClick(e); return dpnla.eventCancel(e); });
			}
			if(fg[0] > 0) mx -= 2;
			this.data[id +'_mx'] = mx;
		},
		tabClick: function(e){
		/* タブクリック */
			var em = e.target.closest('a');
		//	while(em.tagName.toLowerCase() != 'li'){
		//		em = em.parentNode;
		//	}
			var ep = $(em).parent();		var id = ep.attr('id');
		//	var ep = em.parentNode;		var id = ep.id;
			var fg = this.data[id +'_fg'];		var po = 0;
			var tb = this.data[id +'_li'];		var tm = tb.length - 1;
			for(i = 0;i <= tm;i++){
				if(tb[i] == em){
					po = i;		break;
				}
			...
			if(fg[0] > 0){ // if has arrow
				if(po == 0){ // if clicked left arrow
					po = ns - 1; // shift 1
					if(po < 0) po = 0; // if exceeds 0, limit at 0
				}else if(po == tm){ // if clicked right arrow
					po = ns + 1; // unshift 1
					if(po >= mx) po = mx - 1; // if exceeds max length, limit at max length
				}else{
					po--;
				}
			}
			this.tabSelect(id,po);
		},
		...
		tabSelect: function(id,po){
		/* タブ選択 */
		// create css class 'panel' with display: none, then show panel by adding class active with display: block
			var fg = this.data[id +'_fg'];		var i = 0;	var j = 0;	var ky = '';
			var tb = this.data[id +'_li'];		var tm = tb.length - 1;
			console.log(`tb: `);
			console.log(tb);
			if(fg[0] > 0){
		...
		/* タブ本体状態設定 */
			...
			if(fg[1] > 0){ // if 1 tab controls 2 subpanes, set 2nd pane too
				ky += '_a';		oa = this.ge(ky);
				if(oa != undefined) oa.style.display = vl;
			}
		},
		...
		/* 選択記憶＆最大取得 */
			var pa = [];
			var ky = id +'_mx';		pa[0] = this.data[ky];
				ky = id +'_ns';		pa[1] = this.data[ky];
			return pa;
		},
		tmpget: function(id){
		/* テンプレート取得 */
			var ob = $('#'+id);
			if(ob.length == 1) return ob.html().split('<!--_%DLIMT%_-->');
			else return '';
		...
		*/
		...
		/* 戦闘タブ初期化 */
			...
			this.tmpviw(0,'t41_2','');	this.tmpviw(0,'t41_2_a','');	this.tabSelect('t41',0);
		},
		...
		/* 記録消去確認 */
			...
			this.data['kcprecdelarg'] = [pt,ky,ra];
			var ob = this.ge(ra[1]);
			if(ob != undefined){
				ob.disabled = true;		ob.style.visibility = 'hidden';	// 無効化＆非表示
			}
			this.data['pmbcloserunproc'] = function(){
				var pa = dpnla.data['kcprecdelarg'];
			...
			$(ra[1]).click(() => { dpnla.kcprecdelgo(); });
			$(ra[2]).click(() => { dpnla.pmbclose(); });
		...
		/* 記録消去 pt 0:logbook 1:enemy_list 2:enemy_db */
			var va = {};	var pt = this.data['kcprecdelarg'];
			...
			this.data['pmbcloserunproc'] = null;
			this.pmbclose();
		},
		kcprecls: function(){
		/* 記録表示欄クリア */
		...
		/* 開始日時を記憶 */
			...
			b.setTime(a.getTime());
			this.data['kcpstimesavedat'] = b;
		},
		kcpstimeview: function(){
		/* 経過時間を返却 */
			var b = this.data['kcpstimesavedat'];	var a = new Date();
			if(!b) b = new Date();
			if($svDateTime) a = $svDateTime;
		...
		pmbclose: function(){
		/* ポップアップメッセージボックスを閉じる */
			var ky = 'pmbcloserunproc';		var cp = this.data[ky];
			var po = this.ge('pmb01');	var mo = this.ge('pmb03');
			if(cp) cp();
			this.data[ky] = null;
...
$(window).on('load', ()=>{dpnla.init();});/* 初期起動 */
